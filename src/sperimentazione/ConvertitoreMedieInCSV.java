...
public class ConvertitoreMedieInCSV {
    ...
    private static void convertTxtToCsv() throws IOException {
        ...
        try (BufferedWriter csvWriter = Files.newBufferedWriter(Paths.get(OUTPUT_CSV))) {
            ...
            csvWriter.write("Categoria_Griglia,Nome_Griglia,Compito_Due,Compito_Tre,Cache_Usata,Sorted_Frontiera,Media_Cache_Hit,Media_Celle_Frontiera," +
                           ...
                           "Spazio_KB,Massima_Profondita,Esecuzioni_Corrette,Landmark,CDF_Per_Landmark,"
                           + "Timeout,Destinazione_Irraggiungibile");
            csvWriter.newLine();
    ...
    private static void processFile(Path txtFile, BufferedWriter csvWriter) throws IOException {
        ...
        boolean isDestinationUnreachable = DESTINAZIONE_IRRAGGIUNGIBILE_PATTERN.matcher(content).find();
        ...
        if (result != null) {
            ...
            if (numeroLandmark.find() && !isDestinationUnreachable) {
    			...
    			result.numeroLandmark = numeroLandmark.group(1).replace(",", ".");
    ...
    private static ExtractionResult extractValues(String section) {
        ...
        try {
            ...
            if (matcher.find()) {
                result.mediaCacheHit = matcher.group(1).replace(",", ".");
            } else {
                return null;
            }
            ...
            if (matcher.find()) {
                result.mediaCelleFrontiera = matcher.group(1).replace(",", ".");
            } else {
                return null;
            }
            ...
            if (matcher.find()) {
                result.mediaIterazioniCondizione = matcher.group(1).replace(",", ".");
            } else {
                return null;
            }
            ...
            if (matcher.find()) {
                result.massimaProfondita = matcher.group(1).replace(",", ".");
            } else {
                return null;
            }
