...
//			e.printStackTrace();
	...
	private ICammino gestisciInterruzione_GeneraCammino(Throwable e) {
		...
		} else if (this.camminoMinimo != null 
				&& this.camminoMinimo.lunghezza() < Double.POSITIVE_INFINITY) {
			strategies.getDebugStrategy().println("Cammino trovato");
			return this.camminoMinimo;
		} else {
			strategies.getDebugStrategy().println("Calcolo non andato a buon fine");
			return ICammino.INFINITY;
		}
	}
	...
	private void inizializzaMonitors(ICella2D O, ICella2D D) {
		...
		monitorMin.setCammino(ICammino.INFINITY);
	}
	...
	private ICammino calcoloCamminoMin(IGriglia<?> griglia, ICella2D O, ICella2D D, IStatisticheEsecuzione stats,
		...
		List<ICella2D> frontieraList = strategies.getFrontieraStrategy().
		...
		DistanzaLibera lunghezzaMin = DistanzaLibera.INFINITY;
		...
		for (ICella2D F : frontieraList) {
			...
			strategies.getDebugStrategy().println("Frontiera stato: " + bitPrint(F.stato())+" "+F.coordinateToString());
			...
			if (F.isNot(StatoCella.OSTACOLO)) {
				...
				strategies.getDebugStrategy().println("Analizzo cella frontiera"
						+ F.coordinateToString()+ " stato: " + bitPrint(F.stato()));

				var distanzaLibera = DistanzaLibera.from(origine, F);
				double lF = distanzaLibera.distanza();

				boolean condizioneSoddisfatta = strategies.getCondizioneStrategy()
						.isSoddisfatta(F, dest, lF, lunghezzaMin.distanza());
				...
				if (condizioneSoddisfatta) {
					...
					stackCammino.push(new Landmark(StatoCella.LANDMARK.addTo(F.stato()), F.x(), F.y()));

					ICammino camminoFD = calcoloCamminoMin(g2, F, dest, stats, compitoDue);

					DistanzaLibera lTot = distanzaLibera.add(camminoFD.distanzaLibera());

					aggiornaMonitorConNuovaFrontiera();

					int comp = lunghezzaMin.compareTo(lTot);

					if (comp>0 || (comp==0 && camminoFD.landmarks().size() + 1 < seqMin.size())) {
						lunghezzaMin = lTot;

						aggiornaSequenzaMinima(seqMin, O, F, camminoFD);
						aggiornaMonitorMinimo(O, seqMin);
						if(1 == livelloRicorsione)
							this.aggiornaCamminoMinimo(Cammino.from(seqMin));
					}
					stackCammino.pop();
		...
		ICammino risultatoFinale = new Cammino(lunghezzaMin, seqMin);
	...
	private static void aggiornaSequenzaMinima(List<ILandmark> seqMin, ICella2D O, ICella2D F, ICammino camminoFD) {
		seqMin.clear();
	...
	private void aggiornaMonitorMinimo(ICella2D O, List<ILandmark> seqMin) {
		if (O.sameCoordinateAs(monitor.getOrigine()) && config.isMonitorEnabled()) {
			camminoMinimo = Cammino.from(seqMin);

			monitorMin.setCammino(camminoMinimo);
			this.aggiornaCamminoMinimo(camminoMinimo);
		}
	}
	...
	private ICammino gestisciCasoFrontieraVuota(IGriglia<?> griglia, ICella2D O, ICella2D dest) {
		...
		if (config.isMonitorEnabled()) {
			monitor.setCammino(new Cammino(DistanzaLibera.INFINITY, new ArrayList<>(stackCammino)));
		}
	...
	private ICammino gestisciCasoBase(ICella2D O, IGrigliaConOrigine g, ICella2D dest) {
		...
		ICammino risultato = Cammino.from(Arrays.asList(landmarkOrigine, landmarkDestinazione));
	...
	private void aggiornaMonitorConNuovaFrontiera() {
		if (config.isMonitorEnabled()) {
			monitor.setCammino(Cammino.from(new ArrayList<>(stackCammino).reversed()));
		}
	}
