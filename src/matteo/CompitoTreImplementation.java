...
//		Thread.sleep(100);
		...
		for (ICella2D F : frontieraList) {
			...
			if (F.isNot(StatoCella.OSTACOLO)) {
				...
				double IF = distanzaLibera.distanza();
				
				boolean condizioneSoddisfatta = strategies.getCondizioneStrategy()
						.isSoddisfatta(F, dest, IF, lunghezzaMin.distanza());
				...
				if (condizioneSoddisfatta) {
					...
					DistanzaLibera ITot = distanzaLibera.add(camminoFD.distanzaLibera());
					
					aggiornaMonitorConNuovaFrontiera();
					
					int comp = lunghezzaMin.compareTo(ITot);
	...
	private void aggiornaMonitorMinimo(ICella2D O, List<ILandmark> seqMin) {
		if (config.isMonitorEnabled() && O.sameCoordinateAs(monitor.getOrigine())) {
			monitorMin.setCammino(Cammino.from(seqMin));
		}
	}
